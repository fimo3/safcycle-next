...
const Map = ({ startLocation, endLocation, onDataUpdate, mode }) => {
  ...
  // Initialize Leaflet map on client-side and fix default marker icons
  useEffect(() => {
  ...
  // Fetch directions whenever startLocation, endLocation, or mode changes
  useEffect(() => {
    if (mapInstance && startLocation && endLocation) {
      getDirections(startLocation, endLocation)
    }
  }, [mapInstance, startLocation, endLocation, mode])
    ...
    try {
      ...
      if (data.paths && data.paths.length > 0) {
        ...
        import("leaflet").then((L) => {
          ...
          // Add new markers (using default icon configured earlier)
...
// Use Next.js dynamic import to disable SSR for this component
